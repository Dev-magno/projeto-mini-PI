...
Class User {
    ...
    public function __construct($id = false) {
        ...
        if($id){
            $this->usuario_id = $id;//Se chamar um new user e não passar o id, não acontece nada.
            //Se passar, o objeto é inicializado
            $this->carregar(); //Quando o id for passado ao mesmo tempo que atribui um id para o 
            //objeto vai fazer um carregar. Usa o id para carregar o restante das informações
        }
    }
    ...
    // O método setEndereco é usado para definir o valor da propriedade 'endereco' da classe.
    public function setEndereco($endereco) {
        // O parâmetro $endereco representa o novo valor que será atribuído à propriedade 'endereco'.
        
        // A linha abaixo faz a atribuição do valor passado para a propriedade 'endereco' da instância.
        // Em seguida, o método retorna o valor atribuído.
   ...
   public function setSenha($senha) {
    ...
    public  function criar() {
        try {
            ...
            $sql = "INSERT INTO Usuario_tb (nome, endereco, data_nascimento, telefone, cpf, email, senha) VALUES (:nome, :endereco, :data_nascimento, :telefone, :cpf, :email, :senha)";
            $stmt = $conexao->prepare($sql);
            // Associa os valores das propriedades da classe aos parâmetros da consulta SQL
            $stmt->bindValue(':nome', $this->getNome()); // Associa o valor da propriedade 'nome' ao parâmetro ':nome' na consulta SQL
    ...
    public function atualizar() {
        try {
            $conexao = Conexao::conectar(); 
            //A clausa 'SET' especifica quais colunas da tabela usuário 'Usuario_tb' devem ser atualizadas e com quais novos valores.
            $sql = 'UPDATE Usuario_tb SET nome=:nome, endereco=:endereco, data_nascimento=:data_nascimento, telefone=:telefone, cpf=:cpf, email=:email, senha=:senha WHERE usuario_id = :usuario_id';
            $stmt = $conexao->prepare($sql);
            $stmt->bindValue(':nome', $this->getNome()); // nome: A coluna nome será atualizada com o valor fornecido pelo parâmetro :nome.
            $stmt->bindValue(':endereco', $this->getEndereco());
            $stmt->bindValue(':data_nascimento', $this->getDataNascimento());
            $stmt->bindValue(':telefone', $this->getTelefone());
            $stmt->bindValue(':cpf', $this->getCpf());
